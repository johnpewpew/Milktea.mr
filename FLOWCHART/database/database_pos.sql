...
CREATE TABLE `pending_transactions` ( `id` int(11) NOT NULL, `user_id` int(11) DEFAULT NULL, `total_amount` decimal(10,2) DEFAULT NULL, `order_details` text DEFAULT NULL, `transaction_date` timestamp NOT NULL DEFAULT current_timestamp(), `created_at` timestamp NOT NULL DEFAULT current_timestamp() ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci; ALTER TABLE `pending_transactions` ADD PRIMARY KEY (`id`); ALTER TABLE `pending_transactions` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=56 ; SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO'; INSERT INTO `pending_transactions`(`id`, `user_id`, `total_amount`, `order_details`, `transaction_date`, `created_at`) SELECT `id`, `user_id`, `total_amount`, `order_details`, `transaction_date`, `created_at` FROM `pending_transactions`;
